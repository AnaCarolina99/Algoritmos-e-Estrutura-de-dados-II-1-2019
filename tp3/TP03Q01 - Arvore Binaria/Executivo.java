 ...
 */
 class ArvoreBinaria {
     ...
     */
    public String pesquisar(String x) {
        return x + " raiz" + pesquisar(x, raiz);
    }
     ...
     * <code>false</code> em caso contrario.
     */
    private String pesquisar(String x, No i) {
        String resp = " ";
          if (i == null) {
            comp++;
           resp += convertUTF8toISO("NÃO");
          ...
          comp++;
           resp += "esq" + pesquisar(x, i.esq);

        } else {
          comp++;
           resp += "dir" + pesquisar(x, i.dir);
        }
        return resp;
      }
...
//laco para acessar o arquivo e imprimir seus dados
    ...
    for(int i = 0; i<nEntrada2; i++){
      Presidente p = new Presidente();
      selena += arvore.pesquisar(entrada2[i]) + "\n";
