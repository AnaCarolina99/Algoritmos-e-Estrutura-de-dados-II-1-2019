...
class Hash {
   Presidente tabela[];
   int tamTab;
    int comp;//Numero de comparacoes feitas
...
//Construtor
   ...
   public Hash (int tamTab){
      this.tamTab = tamTab;
      this.tabela = new Presidente [this.tamTab];
      comp = 0;
   ...
   public int h(Presidente elemento){
      return elemento.getId() % tamTab;
   }
   //Metodo para retornar a posicao da tabela rehash a ser inserida
   public int reh(Presidente elemento){
      return ((elemento.getId() + 1)) % tamTab;
   }
...
//Metodo para pesquisar se a variavel String existe e retorna um boolean
   public boolean pesquisar (String elemento){
      ...
      if(!achou){
        comp++;
        MyIO.println(elemento + convertUTF8toISO(" NÃO"));
      }
      return achou;
   }
...
//laco para acessar o arquivo e imprimir seus dados
    ...
    Arq.openWrite("591513_hashRehash.txt","ISO-8859-1");
    //Printacao das variaveis contidas no arquivo
    Arq.println("591513 \t Tempo:" + com +"s \t Comparaçoes:" +hash.comp());
