...
class Bolha1{
    ...
    //contrutor
    public Bolha1(int tamanho, Presidente[] array){
        n = tamanho;
        this.array = array;
        numComp = 0;
        numMovi = 0;
    ...
    //metodo para desimpate
    public void sort(){
        for(int i = 0; i < n; i++){
            int menor = i;
            for(int j = (i+1); j < n; j++){
                numComp++;
                if(array[menor].getLocalNascimento().equals(array[j].getLocalNascimento())){
    ...
    public Presidente[] bubbleSort() throws Exception {
        for (int i = n; i >= 1; i--) {
            for (int j = 1; j < i; j++) {
                if (array[j - 1].getLocalNascimento().compareTo(array[j].getLocalNascimento()) > 0) {
                    Presidente aux = array[j];
                    array[j] = array[j - 1];
                    array[j - 1] = aux;
                }  
            }
...
public class Bolha {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Bolha1 s = new Bolha1(lista.getN(), lista.getArray());
        s.sort();
        s.bubbleSort();
        lista.mostrar();
    }
