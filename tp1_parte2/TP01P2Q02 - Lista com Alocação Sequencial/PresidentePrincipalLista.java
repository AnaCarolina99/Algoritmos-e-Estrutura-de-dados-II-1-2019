...
public class PresidentePrincipalLista {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int nEntrada = 0;
        String[] alocacao = new String[1000];
        String[] tmp;
        int nAlocacao = 0;
        ...
        do {
            alocacao[nAlocacao] = MyIO.readLine();
            nAlocacao++;
        } while (nAlocacao < n);
        ...
        for (int j = 0; j < n; j++) {
            tmp = alocacao[j].split(" ");
            Presidente p = new Presidente();
            if (tmp[0].charAt(0) == 'I') {
                if (tmp[0].charAt(1) == 'I') {
                    p.ler(tmp[1]);
                    lista.inserirInicio(p);
                } else if (tmp[0].charAt(1) == 'F') {
                    p.ler(tmp[1]);
                    lista.inserirFim(p);
                    ...
                    MyIO.println("Comando invalido: " + tmp[0]);
                    System.exit(1);
                }
            } else if (tmp[0].charAt(0) == 'R') {
                if (tmp[0].charAt(1) == 'I') {
