       ...
       int j,x;
        for (int i = 1; i < n; i++) {
          Presidente tmp = array[i];
           ...
           for(j = x;(j >= 0) && (array[j].getPaginaTam() > tmp.getPaginaTam());j--) {
             comp++;
              array[j + 1] = array[j];
              j--;
           }//Fim for
...
//Laco para ler vetor de String ate a flag "FIM"
      ...
      //instanciando a classe insercao Parcial, ordenando e mostrando o resultado
      for(int i = 0;i<nEntrada;i++){
        lista.insercao();
      }
      lista.mostrar();
      ...
      Arq.openWrite("591513_insercaoParcial.txt","ISO-8859-1");
