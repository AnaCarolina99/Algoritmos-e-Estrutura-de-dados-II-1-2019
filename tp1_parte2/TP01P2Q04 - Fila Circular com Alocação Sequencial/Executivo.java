...
class Fila {
   ...
   /*Metodo que mede todas as media da variavel idade
   ...
   */
   public int mediaArredondada(){
     int first = primeiro;
      double soma = 0;
      int cont = 0;
        while((first % array.length) != ultimo){
          soma += array[first].getIdade();
          first = (first+1) % array.length;
          cont++;
        }
        return (int)Math.round(soma/cont);
   }
}//Fim pilha
...
//laco para acessar o arquivo e inserir no seu final e depois fazer a mediaArredondada
      for(int i = 0;i<nEntrada;i++){
        Presidente p = new Presidente();
        p.ler(entrada[i]);
        fila.inserir(p);
        MyIO.println(fila.mediaArredondada());
      }

      String[] linha;
      //Laco para instanciar Presidente e inserir ou remover as linhas desejadas
      //Quando inserida , fazer a sua mediaArredondada
