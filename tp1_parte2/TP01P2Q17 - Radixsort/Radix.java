...
class RadixSort {
    ...
    //contrutor
    public RadixSort (int tamanho, Presidente[] array){
        n = tamanho;
        this.array = array;
        numComp = 0;
        numMovi = 0;
    ...
    //metodo compara o maior e menor numero(id)
    public void sort(){
        for(int i = 0; i < n; i++){
            int menor = i;
            for(int j = (i+1); j < n; j++){
    ...
    public void radixsortEp(){
        int s = array[0].getId();
        int exp = 1;

        Presidente[] arrayS = new Presidente[n];
        ...
        while (s / exp > 0){
            ...
            for (int i = 0; i < n ; arrayAux[(array[i].getId() / exp) % 10]++, i++);
            for (int i = 1; i < 10; arrayAux[i] += arrayAux[i - 1], i++);
            for (int i = n - 1; i >= 0; arrayS[--arrayAux[(array[i].getId() / exp) % 10]] = array[i], i--);
            for (int i = 0; i < n ;array[i] = arrayS[i], i++);
            exp *= 10;
        }
        ...
        /*for (int i = 0; i < n; i++){
            for (int j = 0; j < n; j++){
                if( (array[i].getId() == array[j].getId()) ) {
                    if ( array[i].getId().compareTo(array[j].getId()) > 0 ){
                        Presidente tmp1 = array[i];
                        array[i] = array[j];
                        array[j] = tmp1;
                    }
                    if (array[i].getId().compareTo(array[j].getId()) < 0 ){
                        Presidente tmp2 = array[j];
                        array[j] = array[i];
                        array[i] = tmp2;
                    }
                }
...
public class Radix {
    ...
    public static void main(String[] args) throws Exception {
        ...
        RadixSort s = new RadixSort(lista.getN(), lista.getArray());
        s.radixSort();
        lista.mostrar();
    }
