...
class Lista {
   ...
   /*Algoritmo de ordenacao radixsort
    * Ordena a variavel codigo e depois a variavel sigla
    * retorna o numero de comparacoes feitas
    */
        public void radixsortEp(){
            int doido = array[0].getId();
            int exp = 1;

            Presidente[] arrayDoido = new Presidente[n];

            for (int i = 1; i < n; i++){
              if (array[i].getId() > doido ) {
                doido = array[i].getId();
                comp++;
              }
            }//Fim for
            while (doido / exp > 0){
             ...
             for (int i = n - 1; i >= 0; arrayDoido[--arrayAux[(array[i].getId() / exp) % 10]] = array[i], i--);
             for (int i = 0; i < n ;array[i] = arrayDoido[i], i++);
             exp *= 10;
           }//Fim while
       }//Fim metodo
...
//laco para ordenar o arquivo
      ...
      Arq.openWrite("591513_radixsort.txt","ISO-8859-1");
