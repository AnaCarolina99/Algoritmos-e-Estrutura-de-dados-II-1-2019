   ...
   /**
       ...
       * @param int dir fim do array a ser ordenado
       ...
       private void quicksort(int esq, int dir) {
           ...
           while (i <= j) {
             comp++;
               while (array[i].getInicioMandato().compareTo(pivo.getInicioMandato()) < 0 ){
                 comp++;
                 i++;
               }
               while (array[j].getInicioMandato().compareTo(pivo.getInicioMandato()) > 0 ){
                 comp++;
                j--;
               }
       ...
       /**
        * Algoritmo de ordenacao parcial Quicksort.
       * @param int k
        */
       private void quicksortParcial(int k) {

         quicksort(0,n-1);
            for(int i = 0; i < n;i++){
              for(int j = 0; j < k;j++){
                  if(array[i].getInicioMandato().compareTo(array[i].getInicioMandato()) < 0){
                    comp++;
                    swap(i,j);
                  }

                  if(array[j].getInicioMandato().compareTo(array[i].getInicioMandato()) > 0 ){
                    comp++;
                    swap(i,j);
                  }
              }
...
//laco para ordenar o arquivo parcial
      for(int i = 0;i<nEntrada;i++){
        lista.quicksort();
      }
      lista.mostrar();
      ...
      Arq.openWrite("591513_quicksort.txt","ISO-8859-1");
