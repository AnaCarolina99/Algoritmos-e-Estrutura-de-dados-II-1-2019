...
class Lista {
   ...
   */
   ...
   public  void constroi(int tamHeap){
        for(int i = tamHeap; i > 1 && array[i].getDataNascimento().compareTo(array[i/2].getDataNascimento()) >0; i /= 2){
           swap(i, i/2);
        }
     }
    ...
    public void reconstroi(int tamHeap){
        ...
        while(i <= (tamHeap/2)){
           ...
           if (array[2*i].getDataNascimento().compareTo(array[2*i+1].getDataNascimento()) >0 || 2*i == tamHeap){
              filho = 2*i;
              comp++;
           } else {
             comp++;
           ...
           if(array[i].getDataNascimento().compareTo(array[filho].getDataNascimento()) <0 ){
              swap(i, filho);
              i = filho;
              comp++;
           }else{
...
//laco para ordenar o arquivo parcialmente
      for(int i = 0;i<nEntrada;i++){
        lista.heapsort();
      }
      lista.mostrar();
      ...
      Arq.openWrite("591513_heapsort.txt","ISO-8859-1");
